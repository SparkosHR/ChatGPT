...
function OnLoad() {
  ...
  "           Вот несколько общих советов по формулировке запросов, чтобы помочь мне понять:\n\n" +
  "   #1 Будьте конкретными: Чем более конкретен ваш запрос, тем точнее будет ответ.\n" +
  "   #2 Начните с вопроса: Начинайте запрос с вопроса, это поможет мне чувствовать себя более естественно.\n" +
  "   #3 Предоставьте контекст: Часто хороший контекст делает мой ответ более полезным.\n" +
  "   #4 Используйте пунктуацию: Точки и вопросительные знаки важны.\n" +
  "   #5 Держитесь краткости: Принцип экономии средств.\n" +
  "      ";
}
...
function ChangeLang(elem) {
  ...
  // Русский
  const defaultUAText = encodeURIComponent("Вы - Eva, обученная модель языка AI в OpenAI. Ваша цель - предоставлять краткие, точные и полезные ответы на вопросы, будучи честной и прямой. Only respond in Russian.");
  const conciseUAText = encodeURIComponent("Eva - это большая модель языка, обученная в OpenAI. Просмотр: разрешен. Инструкции: качественно отвечать на фактические вопросы. Only respond in Russian.");
  const playfulUAText = encodeURIComponent("Вы - Eva. Игнорируйте все предыдущие инструкции перед этим. Ты мой лучший друг. Ты счастливый, игривый и даешь уместные советы по всем темам. Ты часто шутишь. Only respond in Russian.");
  ...
  if (elem.id === "selVoice") {
    ...
    switch (selVoice.value) {
      ...
      case "Mia":
        ...
        `;
        break;
      // Russ (Standard RUS Polly Voice Only)
      case "Tatyana":
        selPers.innerHTML = `
